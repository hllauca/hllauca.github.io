...
description: "Basis for modeling and constraining garbage outputs"
author:
  - name: Harold Llauca
...
\
This objective might be explained by a **minimization problem**, where the cost function *J* represents errors ($\varepsilon$) between simulations (*Ysim*) and observations (*Yobs*) as described in the following equation:\
\
$$
...
\
The cost function is commonly named the **Objective Function** (*OF*) and *J* might be expressed in terms of some statistical metrics such as the Root Mean Square Error (*RMSE*), where *n* represent the length of the time series and *i* the step.\
 

$$
minOF = minRMSE = min(\sqrt{\frac{1}{n}\sum_{i}^{n}\left ( Y_{sim} - Y_{obs} \right )^2})
$$
...
The following list presents some available functions and packages available in the R language programming:
